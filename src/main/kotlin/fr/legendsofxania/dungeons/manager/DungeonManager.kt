...
object StructureManager {
    ...
    suspend fun setupRooms(
        ...
        for (child in roomEntry.children) {
            setupRooms(player, context, child.get(player, context), placedRooms, roomLocation, bound)
        }
    }

    private suspend fun loadRoom(artifactRef: Ref<RoomArtifact>): Structure? {
        val artifact = artifactRef.entry ?: return null
        val cacheKey = artifact.artifactId
        structureCache[cacheKey]?.let { return it }
        ...
        val tempFile = File.createTempFile("${plugin.dataFolder.path}/temp/${artifact.artifactId}", ".nbt").apply {
